...
function getAllPatternsData(data) {
    return _.map(data.ptr, convertPattern)
}

function getPatternData(data) {
    return convertPattern(data.ptr[0])
}
...
function convertPattern(original) {
    return {
        id: original['pid'][0],
        length: 1*original['ln'][0],
        direction: original['rtdir'][0],
        points: _.map(original.pt, convertPoint)
    }
}

function convertPoint(original) {
    var obj = {
        latitude: 1*original['lat'][0],
        longitude: 1*original['lon'][0],
        type: original['typ'][0]
